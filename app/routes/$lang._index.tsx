...
export const action: ActionFunction = async ({ request }) => {
  ...
  if (typeOperation === "contact") {
    ...
    const formatFrom = formData.get("formatFrom") as string | null;
    ...
    try {
      ...
      if (formatFrom === "pdf") {
        convertedFiles = await convertPDFsToImages(files, format)
      } 
      else {
        if (format === "svg") {
...
export default function IndexPage() {
  ...
  const handleAllAction = (files: File[]) => {
    ...
    formData.append("formatFrom", selectedFormatFrom?.toLowerCase());
